...
import { BREAK_THRESHOLD } from './constants.js';

/**
 * Parses TTML XML string and populates the global state and DOM container.
 * @param {string} xmlString - The TTML XML content.
 * @param {HTMLElement} container - The DOM element where lyrics will be rendered.
 * @param {Function} onSpanClick - Callback for when a lyric span is clicked.
 */
export function parseTTML(xmlString, container, onSpanClick) {
  ...
  Array.from(pEls).forEach((p) => {
    ...
    function makeSpanEl(tok) {
      ...
      if (onSpanClick) spanEl.addEventListener('click', () => onSpanClick(tok.begin));
  ...
  if (state.lines.length === 0) {
    container.innerHTML = `<div class="empty-state">
      ...
      <div class="sub">The file was parsed but contained no word-timed lyric lines.</div>
    </div>`;
  }
